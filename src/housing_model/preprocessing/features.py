...
def build_feature_template(artifact_dir):
    feature_template = {
            ...
            "total": (
                ...
                Encoder(
                    [
                        ToList(),
                        CategoricalFeature(
                            os.path.join(artifact_dir, "total_parking"), 10
                        ),
                    ],
                    "int32",
                    1,
            ...
            "garage_type": (
                ...
                Encoder(
                    [
                        ...
                        ToList(),
                        CategoricalFeature(
                            os.path.join(artifact_dir, "municipal"), 100
                        ),
                    ],
                    "int32",
                    1,
            ...
            "garage": (
                ...
                Encoder(
                    [
                        ToList(),
                        CategoricalFeature(
                            os.path.join(artifact_dir, "total_garage"), 10
                        ),
                    ],
                    "int32",
                    1,
        ...
        "price_int": (
            739900,
            0,
            Encoder([Scale(1 / 1e6), PositionEncoder(100, 20 ** 2)], "float32", 1),
        ),
        "price_sold_int": (739900, 0, Encoder([ToList()], "int32", 1)),
        "map": {
            "lat": (
                43.8934482,
                0,
                Encoder([Scale(100), PositionEncoder(100, 1e8)], "float32", 1),
            ),
            "lon": (
                -79.4628076,
                0,
        ...
        "house_area": {
            "estimate": (
                None,
                0,
                Encoder([Scale(1e-3), PositionEncoder(100, 100 ** 2)], "float32", 1),
            )
        },
        "land": {
            "front": (
                39,
                0,
                Encoder([Scale(1 / 10), PositionEncoder(100, 100 ** 2)], "float32", 1),
            ),
            "depth": (
                131,
                0,
        ...
        if isinstance(value, dict):
            ...
            if isinstance(a_listing, dict) or a_listing is None:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Cannot encode {feature_name} with value of {feature_value}"
                    ) from e
            else:
    ...
    with open(in_file_path) as f_in:
        for l in f_in:
            ...
            try:
                ...
                err += 1
    _logger.info(
        f"Cannot convert {err} listings, out of {total_cnt} at the second pass"
    )
